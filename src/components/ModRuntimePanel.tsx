...
export default function ModRuntimePanel({ token, onAuthorizationExpired, onLibraryChanged }: {
  ...
  const closeConfirmation = useCallback(() => { if (!pending) setConfirmation(false); }, [pending]);
  const dialogRef = useDialogFocus(confirmation, closeConfirmation);
  ...
  useEffect(() => {
    setData(null); setConfirmation(false);
    void load();
    return invalidate;
  }, [load, invalidate]);
    ...
    {target && <>
      ...
      {administrator && <div className="mt-4 border-t app-border pt-4">
        <div className="flex flex-wrap items-end gap-3">
          <label className="w-full min-w-0 text-sm app-text-secondary sm:w-auto sm:flex-1">准备运行环境<select aria-label="选择实例 Mod" className="app-control mt-2 block w-full rounded-lg border px-3 py-2" value={modId} disabled={pending || busy} onChange={event => setModId(event.target.value)}>{MOD_CATALOG.filter(item => item.sha).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          ...
          <button type="button" className="app-control rounded-lg border px-4 py-2 text-sm" disabled title="当前实例兼容性验收与应用适配器尚未完成">应用到实例</button>
        </div>
